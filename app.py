...
# API configuration
SPOONACULAR_API_KEY = os.getenv('SPOONACULAR_API_KEY', '86da8f7e6cf84b6895ea78e5186282ec')
...
def recipe():
    ...
    db_results = conn.execute("SELECT * FROM recipes").fetchall()
    all_recipes = list(db_results)
    ...
    for r in api_recipes:
        bad_phrases = ["squeeze", "add ", "stir ", "cover ", "mix ", "simmer ", "heat", "water", "pepper if"]
        ...
        instructions = details.get('instructions', 'No preparation steps available.')

        existing = conn.execute("SELECT id FROM recipes WHERE name = ?", (r['title'],)).fetchone()

        if not existing:
            conn.execute(
                "INSERT INTO recipes (name, image, used_ingredients, missed_ingredients, instructions) VALUES (?, ?, ?, ?, ?)",
    ...
    db_results = conn.execute(
        "SELECT * FROM recipes WHERE used_ingredients LIKE ?",
        ("%" + user_ingredients[0] + "%",)
    ).fetchall()
    
    new_results = conn.execute("SELECT * FROM recipes").fetchall()
    all_recipes = list(new_results)
    conn.close()
